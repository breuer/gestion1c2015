...
namespace PagoElectronico.Depositos
{
    public partial class FormDepositos : Form
    {
        ...
        private void cargar_combo_tarjetas()
        {
            ...
            {
                MessageBox.Show("No hay ninguna tarjeta cargada, no se podran hacer un deposito sin alguna de ellas, haz click en Tarjetas y vincula una.");
                this.Close();
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
              ...
              if ((ix_cuenta >= 0) && (ix_tarj >= 0) && (txt_importe.Text != ""))
              {
                  //Reviso si esta bien el numero.
                  if (float.TryParse(this.txt_importe.Text, out f))
                  {
                      importe = float.Parse(txt_importe.Text);
                  ...
                  {
                      MessageBox.Show("El importe debe ser un valor númerico.");
                  }
                }
                else
                {
                    MessageBox.Show("Es necesario seleccionar una tarjeta y una cuenta para poder seguir ademas del importe a depositar.");
                }
        }
